...
var sidebar = require("sdk/ui/sidebar").Sidebar({
	...
	tabs.on("activate", function() {
		...
		if (selectionInProgressTabs[i].tabId === tabs.activeTab.id) {
		    ...
		    console.log("activeTab  on activate  = "+tabs.activeTab.id);
		    console.log("selectionInProgressTab on activate  = "+selectionInProgressTabs[i].tabId);
		    break;
		}
	    }
...
/*	tabs.on("deactivate", function() {
	...
	worker.port.on("unchecked", function () {
	    for (var i=0;i<selectionInProgressTabs.length;i++){
		if (selectionInProgressTabs[i].tabId === tabs.activeTab.id) {
		    console.log("selectionInProgressTab on unchecked  = "+selectionInProgressTabs[i].tabId);
		    console.log("activeTab  on unchecked  = "+tabs.activeTab.id);
